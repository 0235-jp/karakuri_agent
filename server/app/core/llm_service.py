...
from litellm import (
    ...
    ChatCompletionUserMessage, ChatCompletionSystemMessage,
    Choices, CustomStreamWrapper, acompletion, ModelResponse
)
...
{{
    ...
    async def generate_response(
        ...
        conversation_history: Optional[List[AllMessageValues]] = None,
        image: Optional[bytes] = None
    ) -> LLMResponse:
        messages: List[AllMessageValues] = []
        
        if agent_config.llm_system_prompt:
            messages.append(ChatCompletionSystemMessage(
                role="system",
        ...
        if image:
            ...
            if not CHECK_SUPPORT_VISION_MODEL or litellm.supports_vision(model=agent_config.message_generate_llm_model):
                messages.append(ChatCompletionUserMessage(role="user", content=[
    ...
    async def convert_images_to_text(self, data_url: str, agent_config: AgentConfig) -> str:
        ...
        vision_messages.append(ChatCompletionSystemMessage(
            role="system",
            content="You are a vision model that converts images into a descriptive text."
        ))
        vision_messages.append(ChatCompletionUserMessage(
            role="user",
            content=[
                ChatCompletionImageObject(
                    type="image_url",
    ...
    async def generate_emotion_response(
        ...
        emotion_messages.append(ChatCompletionSystemMessage(
            role="system",
            content="You are an expert emotion analyzer. Always respond in the exact JSON format requested.",
        ))
        emotion_messages.append(ChatCompletionUserMessage(
            role="user",
